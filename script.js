...
function renderCourseStateList(container,courses,kind){
  ...
  if(courses.length>visible.length){
    ...
    more.textContent=`+${courses.length-visible.length} ${uiText("iitmLearning.more","more in the curriculum map below")}`;
...
function populateIitmCourseDetails(details,group,curriculum){
...
function renderIitmLearning(curriculum,state){
  ...
  if(qualifier){
    ...
    strong.textContent=uiText("iitmLearning.qualifierLabel","OFFICIAL QUALIFIER CONTEXT");
    const p=document.createElement("p");
    const names=(curriculum.qualifierContext?.courseCodes||[])
      .map(code=>curriculum.courses?.[code]?.name)
      .filter(Boolean);
  ...
  if(progression){
    ...
    (curriculum.conceptualProgression?.steps||[]).forEach((step,index,steps)=>{
      ...
      if(index<steps.length-1){
        const i=document.createElement("i");i.textContent="→";i.setAttribute("aria-hidden","true");progression.append(i);
      }
    });
    const note=document.createElement("small");
    note.textContent=curriculum.conceptualProgression?.label||"";
  ...
  if(layers){
    ...
    for(const [index,group] of groups.entries()){
      ...
      article.className="iitm-layer";
      article.dataset.layer=group.id;
      ...
      level.textContent=`${uiText("iitmLearning.portfolioGroup","PORTFOLIO VIEW")} · ${officialLevels.join(" / ")}`;
      title.append(h3,level);head.append(num,title);
      article.append(head);

      const meaning=document.createElement("div");meaning.className="iitm-meaning-grid";
      const why=document.createElement("p");
      const whyLabel=document.createElement("b");whyLabel.textContent=uiText("iitmLearning.why","WHY IT MATTERS");
      ...
      const connectionLabel=document.createElement("b");connectionLabel.textContent=uiText("iitmLearning.connection","PORTFOLIO CONNECTION");
      ...
      summary.textContent=`${uiText("iitmLearning.representativeCourses","Representative official courses")} · ${groupCourses.length}`;
      details.append(summary);
      details.addEventListener("toggle",()=>{
        if(details.open&&!details.dataset.rendered) populateIitmCourseDetails(details,group,curriculum);
      });
  ...
  if(connections){
    ...
    for(const connection of curriculum.academicConnections||[]){
      ...
      for(const target of connection.targets||[]){
        ...
        a.href="#"+target;
        a.textContent=(targetEl.querySelector("h3")?.textContent||target)+" ↓";
        links.append(a);
      }
      article.append(head,note,links);
  ...
  if(intersection){
    ...
    for(const area of curriculum.longTermIntersection?.areas||[]){
      const span=document.createElement("span");span.textContent=area;intersection.append(span);
    }
  }
